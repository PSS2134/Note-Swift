...
const EditModal = ({ closeEditModal, id }) => {
  ...
  return (
    <div className="flex justify-center items-center fixed top-0 right-0 left-0 backdrop-blur-sm h-screen">
      <div className="h-fit w-[98%] p-5 md:w-[60%] md:p-[2vh] md:px-[5vh] m-2 bg-amber-200 rounded-md shadow-md shadow-black backdrop-blur-3xl">
        <img
          src={cross}
          onClick={() => {
            closeEditModal();
        ...
        <p className="text-xl md:text-3xl mt-10 font-semibold ">Title</p>
        <input
          className="mt-2 rounded-md p-2 w-[100%] text-lg md:text-2xl  focus:outline-amber-400"
          placeholder="A Beautiful Day..."
          name="Title"
          value={note.Title}
          onChange={handleNote}
        />
        <p className="text-md md:text-lg font-semibold m-1 text-gray-600">
          {note.Title.length}/30 characters
        </p>
        <p className="text-xl md:text-3xl mt-6 mb-2 font-semibold">Body</p>
        <div>
          <div className="flex p-1 bg-white border-b-2 border-amber-400">
            <img
              src={bold}
              className={`h-[3.5vh] md:h-[5vh] mx-2 hover:bg-amber-200 cursor-pointer p-1 rounded-sm ${
          ...
          <textarea
            rows={8}
            className="pt-2 rounded-b-md p-2 w-[100%] text-lg md:text-2xl  focus:outline-amber-400"
        ...
        <button
          className="text-xl md:text-2xl p-2 px-10 md:mt-0 mt-2 bg-blue-600 text-white rounded-md shadow-md shadow-stone-700 hover:bg-blue-700 active:shadow-none block mx-auto"
          onClick={handleSave}
        >
          Save
        </button>
